...
class AppDatabase {
  ...
  Future<Database> _initDb() async {
    ...
    return openDatabase(
      ...
      onUpgrade: (db, oldV, newV) async {
        if (oldV < 2) {
          // group 是保留关键字 → cat_group
          await db.execute('DROP TABLE IF EXISTS budget_categories');
          await db.execute('DROP TABLE IF EXISTS budget_records');
          await db.execute('''
  ...
  Future<void> _createTables(Database db) async {
    await db.execute('''
      CREATE TABLE IF NOT EXISTS beiyun_tasks(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL,
        stage TEXT NOT NULL,
        done INTEGER DEFAULT 0,
    ...
    ''');
    await db.execute('''
      CREATE TABLE IF NOT EXISTS beiyun_finance(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        amount REAL NOT NULL,
        date TEXT NOT NULL,
        note TEXT DEFAULT '',
